        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <form action="{{ route('admin.doctors.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="col-sm-12">
                            <div class="mb-3">
                                ...
                                <input name="name" type="text" class="form-control @error('name') is-invalid @enderror"
                                    placeholder="Insert doctor name here.." value="{{ old('name') }}">
                                @error('name')
                            ...
                            <div class="mb-3">
                                ...
                                <select name="specialist" class="js-example-basic-single form-select @error('specialist') is-invalid @enderror" data-width="100%">
                                    ...
                                    @foreach ($specialists as $item)
                                        <option value="{{ $item->id }}"{{ old('specialist') == $item->id ? 'selected' : null }}>{{ $item->name }}</option>
                                    @endforeach
                                </select>
                                @error('specialist')
                                    <div class="invalid-feedback">{{ $message }}</div>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="sub_specialist" class="js-example-basic-single form-select @error('sub_specialist') is-invalid @enderror" data-width="100%">
                                    ...
                                    @foreach ($subSpecialists as $item)
                                        <option value="{{ $item->id }}"{{ old('sub_specialist') == $item->id ? 'selected' : null }}>{{ $item->name }}</option>
                                    @endforeach
                                </select>
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="mb-3">
                                ...
                                <textarea name="keterangan" class="form-control @error('keterangan') is-invalid @enderror" rows="2" placeholder="Insert keterangan here..">{{ old('keterangan') }}</textarea>
                                @error('keterangan')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="site" class="js-example-basic-single form-select @error('site') is-invalid @enderror" data-width="100%">
                                    ...
                                    @foreach ($sites as $item)
                                        <option value="{{ $item->id }}"{{ old('site') == $item->id ? 'selected' : null }}>{{ $item->name }}</option>
                                    @endforeach
                                </select>
                                @error('site')
                                    <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="text-end">
                        <button name="btnCreateSimpan" class="btn btn-warning" type="submit">Save and Create Another</button>
